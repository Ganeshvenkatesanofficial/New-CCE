...
function startScrambling() {
  const scrambleIntervalId = setInterval(scrambleText, 50);

  // Stop scrambling after 2 seconds
  setTimeout(() => {
    clearInterval(scrambleIntervalId);
    textElement.innerHTML = originalText; // Restore the original text
  }, 2000);
}

function startProcess() {
  // Start the scrambling process every 5 seconds
  const intervalId = setInterval(startScrambling, 5000);

  // Stop the entire process after 20 seconds
  setTimeout(() => {
